...
export function Projects() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const results = await client.fetch(`*[_id == 'dbdb1295-cba9-4a60-aecc-f2bef4f2e92b'] { body } `);
      setFeatured(results);
      const arrayOfIDs = featured[0] && featured[0].body.map((ele:any) => {
        return ele._ref
      })
      // fetch for the data of each post within the list (arrayOfIDs) .
      const postResults = await client.fetch(`*[_id in ${JSON.stringify(arrayOfIDs)}] {
        _id,
        heroImage {
          asset -> {url}
        },
      ...
      // sort the data
      const sortedPosts = postResults.sort((a, b) => {
